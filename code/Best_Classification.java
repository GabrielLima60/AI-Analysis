...
public class Best_Classification extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {    
        if (startPanel.isVisible()) {
            ...
            try{
                ProcessBuilder pbdb = new ProcessBuilder("python", "code\\database.py", username, new String(password), "insert");
                pbdb.redirectErrorStream(true);
                Process process = pbdb.start();

                int exitCode = process.waitFor();
                    if (exitCode != 0) {
                        JOptionPane.showMessageDialog(this, "Error database script", "Error", JOptionPane.ERROR_MESSAGE);
                    }
            }
            ...
            try{
                ProcessBuilder pbdb = new ProcessBuilder("python", "code\\database.py", username, new String(password), "check");
                pbdb.redirectErrorStream(true);
                Process process = pbdb.start();
